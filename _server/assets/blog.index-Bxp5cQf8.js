...
import { g as Route, i as blogCategoriesQuery, j as blogTagsQuery, k as blogListQuery, l as PER_PAGE, B as Button } from "./router-COYVRa-T.js";
...
function BlogIndexPage() {
  ...
  const {
    data: categories = loaderData.categories
  } = useQuery({
    ...blogCategoriesQuery(),
    initialData: loaderData.categories
  });
  const {
    data: tags = loaderData.tags
  } = useQuery({
    ...blogTagsQuery(),
    initialData: loaderData.tags
  });
  const listOptions = blogListQuery({
    search: search.q,
